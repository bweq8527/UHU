...
Button.size=30                          --|大小
Button.icon_normal=nil                  --|图标（通常情况）
Button.icon_undermouse=nil              --|图标（鼠标悬停）
Button.icon_pressed=nil                 --|图标（鼠标按下）
Button.name="Func"                      --|名称 
...
function Button:init(x,y,size,icon,name)
    ...
    self.name=name
    self.MouseState=self:mouseState()
end
--#1.2更新
function Button:update()
    self:mouseState()                 --|判断鼠标与按钮的交互关系
    love.graphics.print("**mousestate:"..type(self.MouseState)..":"..self.MouseState.."**")
...
function Button:mouseState()
    ...
    if  nowX>Button.pos.x and nowX<Button.pos.x+Button.size and nowY>Button.pos.y and nowY<Button.pos.y+Button.size then
        if nowMouse then
            return MouseStates[3]               --*1.鼠标按下按钮
        else
            return MouseStates[2]               --*2.鼠标悬停在按钮上    
        end
    else
        return MouseStates[1]                   --*3.鼠标与按钮无交互
    end
end
--鼠标与按钮交互的动效
...
function Button:drawIcon(Mouse_State)
    ...
    ↓↓]]
    love.graphics.rectangle("line", Button.pos.x, Button.pos.y, Button.size, Button.size)
--    local cases=
--    {
...
function Button:scaleCult(image)
    local iconSize={x=60,y=60}    --图标大小固定
...
function Button:action_dodge()
    ...
    --*1.如果鼠标位于按钮附近（图标外围50%区域）,执行以下代码。但距离中心特别近将会导致缩放比例过大，设置一个上限
    if  trueX>icon_basePoint[1]-self.size and trueX<icon_basePoint[1]+self.size and trueY>icon_basePoint[2]-self.size and trueY<icon_basePoint[2]+self.size then
        ...
        local scale=math.min(1/distance,5)                                                  --计算缩放比例（按照引力场的平方反比关系），上限为5倍    
