local an_Background4=Animation:new(bg4,0,0,1,1,timeline,4,true)

--重写背景的update方法，使其可以最佳适应窗口（居中显示，不拉伸）
function an_Background4:update()
    if self.FLAG then
...
--↓↓由于文件作用域的限制，在这个文件中定义的局部变量an_Background4无法被外部访问，所以需要提供一个访问接口
function animation_Background4()
    return an_Background4
end
--↑↑这个接口的作用是返回实例化后的Animation类（an_Background4）