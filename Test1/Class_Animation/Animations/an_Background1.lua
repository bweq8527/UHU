local an_Background1=Animation:new(bg1,0,0,1,1,timeline,30,true)

--重写背景的update方法，使其可以贴合窗口大小
function an_Background1:update()
    if self.FLAG then
...
--↓↓由于文件作用域的限制，在这个文件中定义的局部变量an_Background1无法被外部访问，所以需要提供一个访问接口
function animation_Background1()
    return an_Background1
end
--↑↑这个接口的作用是返回实例化后的Animation类（an_Background1）